...
private:
    void draw_at(ImDrawList* d, float cx, float cy, const Config& c) {
        ...
        case 4: draw_cross(d, cx, cy, c, false); draw_circle(d, cx, cy, c); break;
        ...
        if (c.dot && c.shape != 3) {
            if (c.outline)
                d->AddCircleFilled({cx, cy}, c.dot_size + c.outline_thickness, c.outline_color, 20);
            d->AddCircleFilled({cx, cy}, c.dot_size, c.color, 20);
        }
    }

    // Symmetric rect centered on a point
    // All measurements go outward from center equally in both directions
    static void draw_cross(ImDrawList* d, float cx, float cy,
        ...
        if (c.outline) {
            ...
            // Right
            d->AddRectFilled({cx+gap-o,    cy-ht-o}, {cx+gap+len+o, cy+ht+o}, oc);
            // Left
            d->AddRectFilled({cx-gap-len-o, cy-ht-o}, {cx-gap+o,    cy+ht+o}, oc);
            // Down
            d->AddRectFilled({cx-ht-o, cy+gap-o},    {cx+ht+o, cy+gap+len+o}, oc);
            // Up
            if (!t_shape)
                d->AddRectFilled({cx-ht-o, cy-gap-len-o}, {cx+ht+o, cy-gap+o}, oc);
        }
    ...
    static void draw_circle(ImDrawList* d, float cx, float cy, const Config& c) {
        // Radius based on gap + half of size so circle sits around the cross arms
        float radius = c.gap + c.size * 0.5f;
        if (c.outline)
            d->AddCircle({cx, cy}, radius, c.outline_color, 32, c.thickness + c.outline_thickness * 2);
        d->AddCircle({cx, cy}, radius, c.color, 32, c.thickness);
    }

    static void draw_dot_only(ImDrawList* d, float cx, float cy, const Config& c) {
        if (c.outline)
            d->AddCircleFilled({cx, cy}, c.size + c.outline_thickness, c.outline_color, 20);
    ...
    static void draw_diamond(ImDrawList* d, float cx, float cy, const Config& c) {
        ...
        if (c.outline)
            for (int i = 0; i < 4; i++)
                d->AddLine(p[i], p[(i+1)%4], c.outline_color, c.thickness + c.outline_thickness * 2);
        for (int i = 0; i < 4; i++)
    ...
    static void draw_arrows(ImDrawList* d, float cx, float cy, const Config& c) {
        ...
        if (c.outline)
            for (auto& seg : segs)
                d->AddLine(seg.a, seg.b, c.outline_color, c.thickness + c.outline_thickness * 2);
        for (auto& seg : segs)
